...
**akashic-typescript**はTypeScriptでAkashicのゲームを作る際のテンプレートです。

## 利用方法

 `akashic-typescript` を利用するにはNode.jsが必要です。

初回のみ、以下のコマンドを実行して、ビルドに必要なパッケージをインストールしてください。
この作業は `akashic-typescript` を新しく生成するごとに必要です。
...
`akashic-typescript` はTypeScriptで書かれているため、以下のコマンドでJavaScriptファイルに変換する必要があります。
...
* ゲーム部分を作成する場合は、 `src/sceneTitle.ts` および `src/sceneGame.ts` を編集してください。
...
`akashic-typescript` でnpmモジュールを利用する場合、このディレクトリで `akashic install <package_name>` することで npm モジュールを追加することができます。

また `akashic uninstall <package_name>` すると npm モジュールを削除することができます。

## エクスポート方法

`akashic-typescript` をエクスポートするときは以下のコマンドを利用します。
